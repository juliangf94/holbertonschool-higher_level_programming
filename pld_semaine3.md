...
No. Python does not have a built-in do...while structure. To mimic it, you usually use a while True loop with a break condition at the end.

##  2. What is a docstring and how do you write one?
A docstring (documentation string) is a string literal that appears as the first statement in a module, function, class, or method definition. It is used to explain what the code does. You write it using triple quotes (""" or ''').
...
Historically, no. However, since Python 3.10, a feature called Structural Pattern Matching was introduced using match and case. Before 3.10, developers used if-elif-else or dictionaries to handle multiple cases.
...
import module: Imports the whole module. You must access functions using the dot notation (module.function()). This is safer and prevents naming conflicts.

from module import *: Imports every public function/variable directly into your workspace. This is generally discouraged because it can overwrite existing names in your script and makes code harder to read.
...
It is a null operation. It does nothing. It is used as a placeholder when a statement is syntactically required but you don’t want to execute any code (like in an empty function or class).
...
No. Python does not have increment/decrement operators like C. You must use i += 1 or i -= 1.
...
A traceback is the report Python gives you when an error occurs. It shows the "trail" of where the error happened, including the file name, the line number, and the sequence of function calls that led to the problem.
